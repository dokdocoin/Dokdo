...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DOKDO")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Dokdo://%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DOKDO unit input in lnReqAmount, we generate the URI with DOKDO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DOKDO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
